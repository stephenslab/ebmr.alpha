...
h1_func = function(fit){
  -(0.5/fit$residual_variance) * (sum((fit$y - fitted.values(fit))^2) +
                                  sum(fit$mu^2/fit$wbar))
}

# This function is not directly used in the algorithm - only for testing.
h2_func = function(fit){
  XtX = svd2XtX(X.svd)
  -0.5*sum((XtX + diag(1/fit$wbar)) * fit$Sigma_full) +
      0.5*as.numeric(determinant(fit$Sigma_full,logarithm = TRUE)$modulus)
}
